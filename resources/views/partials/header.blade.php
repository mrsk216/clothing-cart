        ...
        <div class="flex items-center justify-between gap-4">
            ...
            <a href="{{ route('home') }}" class="flex items-center gap-2 shrink-0 group">
                ...
                <div class="hidden sm:block">
                    <h1 class="text-lg font-serif font-bold text-primary leading-tight bg-gradient-to-r from-primary to-accent bg-clip-text text-transparent">{{ $siteName() }}</h1>
            ...
            <div class="flex items-center gap-3">
                <!-- Wishlist -->
                <a href="{{ route('wishlist') }}" class="relative p-2 text-gray-600 hover:text-primary transition-colors">
                ...
                <!-- Cart -->
                <a href="{{ route('cart') }}" class="relative p-2 text-gray-600 hover:text-primary transition-colors">
                    <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <div class="relative" x-data="{ open: false }">
                            <button @click="open = !open" class="flex items-center gap-2 p-2 text-gray-600 hover:text-primary transition-colors">
                                <div class="w-8 h-8 rounded-full bg-primary/10 flex items-center justify-center">
                                    <span class="text-sm font-semibold text-primary">{{ substr(Auth::user()->name, 0, 1) }}</span>
                                </div>
    ...
    <nav class="hidden md:block border-t border-gray-100 bg-white">
        <div class="max-w-7xl mx-auto px-4">
            <ul class="flex items-center gap-1">
                <li>
                    <a href="{{ route('home') }}" class="fashion-nav-link block px-4 py-2.5">
                        Home
                    </a>
                </li>
                <li>
                    <a href="{{ route('shop') }}" class="fashion-nav-link block px-4 py-2.5">
                        All Products
                    </a>
                </li>
                    ...
                    <div class="absolute left-0 mt-0 w-56 bg-white rounded-lg shadow-lg border border-gray-100 py-2 hidden group-hover:block z-50">
                        @foreach($categories as $category)
                            <div class="relative group/sub">
                                <a href="{{ route('shop', ['category' => $category->slug]) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-primary">
                                    {{ $category->name }}
                                </a>
                                @if($category->children->count() > 0)
                                    <div class="absolute left-full top-0 w-56 bg-white rounded-lg shadow-lg border border-gray-100 py-2 hidden group-hover/sub:block">
                                        @foreach($category->children as $child)
                                            <a href="{{ route('shop', ['category' => $child->slug]) }}" class="block px-4 py-2 text-sm text-gray-700 hover:bg-gray-50 hover:text-primary">
                                                {{ $child->name }}
                                            </a>
                                        @endforeach
                                    </div>
                ...
                <li>
                    <a href="{{ route('about') }}" class="fashion-nav-link block px-4 py-2.5">
                        About
                    </a>
                </li>
                <li>
                    <a href="{{ route('blog') }}" class="fashion-nav-link block px-4 py-2.5">
                        Blog
                    </a>
                </li>
                <li>
                    <a href="{{ route('contact') }}" class="fashion-nav-link block px-4 py-2.5">
                        Contact
                    </a>
                </li>
            </ul>
